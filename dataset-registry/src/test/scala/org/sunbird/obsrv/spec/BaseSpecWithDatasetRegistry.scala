...
class BaseSpecWithDatasetRegistry extends BaseSpecWithPostgres {
  ...
  private def createSchema(postgresConnect: PostgresConnect): Unit = {
    ...
    postgresConnect.execute("CREATE TABLE IF NOT EXISTS datasources ( id text PRIMARY KEY, dataset_id text REFERENCES datasets (id), ingestion_spec json, datalake_ingestion_spec json, datasource text NOT NULL, datasource_ref text NOT NULL, retention_period json, archival_policy json, purge_policy json, backup_config json NOT NULL, status text NOT NULL, created_by text NOT NULL, updated_by text NOT NULL, created_date Date NOT NULL, updated_date Date NOT NULL );")
