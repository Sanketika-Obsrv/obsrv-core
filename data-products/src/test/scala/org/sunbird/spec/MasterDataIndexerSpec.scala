...
class MasterDataIndexerSpec extends FlatSpec with BeforeAndAfterAll with Matchers {
  ...
  private def createSchema(postgresConnect: PostgresConnect) {
    // api_version/entry_topic added to match insertTestData() below, which has always inserted
    // into both columns - this table definition was simply missing them (never caught before,
    // since mvn clean install never previously reached this module: the reactor always failed
    // earlier, on an unrelated pre-existing compile/test issue in a module built before this one).
