...
class BaseSpecWithDatasetRegistry extends BaseSpecWithPostgres {
  ...
  private def insertTestData(postgresConnect: PostgresConnect): Unit = {
    ...
    postgresConnect.execute("insert into dataset_transformations values('tf2', 'd1', 'dealer.maskedPhone', '{\"type\":\"mask\",\"expr\": \"dealer.phone\"}', 'Live', 'Strict', 'System', 'System', now(), now());")
