...
object OTelExecutor {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val openTelemetry = OTelService.init("http://0.0.0.0:4317")
    val logger: OTelLogger = new OTelLogger(LoggerFactory.getLogger(classOf[Example]))
    logger.info("A slf4j log message without a span")
