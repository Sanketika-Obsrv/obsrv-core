...
FROM obsrv/apache-flink:2.0.0-scala_2.12 AS lakehouse-connector-image
USER flink
...
RUN wget https://repo1.maven.org/maven2/org/apache/flink/flink-s3-fs-hadoop/2.0.0/flink-s3-fs-hadoop-2.0.0.jar
RUN wget https://repo.maven.apache.org/maven2/org/apache/hudi/hudi-flink2.0-bundle_2.12/0.15.0/hudi-flink2.0-bundle_2.12-0.15.0.jar
RUN mv flink-shaded-hadoop-2-uber-2.8.3-10.0.jar $FLINK_HOME/lib
RUN mv flink-s3-fs-hadoop-2.0.0.jar $FLINK_HOME/lib
RUN mv hudi-flink2.0-bundle_2.12-0.15.0.jar $FLINK_HOME/lib
...
FROM obsrv/apache-flink:2.0.0-scala_2.12 AS cache-indexer-image
USER flink
RUN mkdir -p $FLINK_HOME/usrlib
