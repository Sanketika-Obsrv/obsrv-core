# Summarizer Flink Job

This job is used to summarize data and extract metrics from the session by running the job using Flink. It can be run either on Yarn or Kubernetes.
...
1. Download flink-1.17.2-scala_2.12 from [apache-flink-downloads](https://www.apache.org/dyn/closer.lua/flink/flink-1.17.2/flink-1.17.2-bin-scala_2.12.tgz). 
...
```
./bin/flink run -m yarn-cluster -p 2 -yjm 1024m -ytm 1024m <data-pipeline-src-dir>/summarizer/target/summarizer-0.0.1.jar
```
...
./bin/flink run -m localhost:8081 <data-pipeline-src-dir>/summarizer/target/summarizer-0.0.1.jar
