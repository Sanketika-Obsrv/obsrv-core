...
class TransformerStreamTestSpec extends BaseSpecWithDatasetRegistry {
  ...
  private def insertTestData(): Unit = {
    ...
    postgresConnect.execute("insert into dataset_transformations values('tf3', 'd2', 'tfdata.valueAsInt', '{\"type\":\"jsonata\",\"expr\":\"$number(id)\"}', 'Live', null, 'System', 'System', now(), now());")
    postgresConnect.execute("insert into dataset_transformations values('tf4', 'd2', 'tfdata.encryptEmail', '{\"type\":\"encrypt\",\"expr\": \"dealer.email\"}', 'Live', 'Lenient', 'System', 'System', now(), now());")
    postgresConnect.execute("insert into dataset_transformations values('tf5', 'd4', 'tfdata.expr1', '{\"type\":\"jsonata\",\"expr\":null}', 'Live', null, 'System', 'System', now(), now());")
    postgresConnect.execute("insert into dataset_transformations values('tf6', 'd4', 'tfdata.expr2', '{\"type\":\"jsonata\",\"expr\":\"$keys(dealer).length\"}', 'Live', null, 'System', 'System', now(), now());")
    postgresConnect.execute("insert into dataset_transformations values('tf7', 'd4', 'tfdata.expr3', '{\"type\":\"jsonata\",\"expr\":\"number(id)\"}', 'Live', null, 'System', 'System', now(), now());")
